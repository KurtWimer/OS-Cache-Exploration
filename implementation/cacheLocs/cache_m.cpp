...
size_t drive::seek_to(size_t block){
    const size_t cache_offset = (disk_size/track_size)/(size_t) 2;
    size_t distance;
    ...
    if((loc = my_organ->find(block) != SIZE_MAX)){
        ...
        loc =  block/track_size;
        //add cache size if above cahce
        if(loc>cache_offset) loc += (my_organ->get_size())/track_size;
    }
